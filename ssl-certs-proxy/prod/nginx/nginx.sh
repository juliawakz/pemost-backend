...
set -e
export DOMAINS="pemost.georesearch.co.ke"
...
use_dummy_certificate() {
  if grep -q "/etc/letsencrypt/live/$1" "/etc/nginx/conf.d/local.conf"; then
    echo "Switching Nginx to use dummy certificate for $1"
    sed -i "s|/etc/letsencrypt/live/$1|/etc/nginx/sites/ssl/dummy/$1|g" "/etc/nginx/conf.d/local.conf"
  fi
}

use_lets_encrypt_certificate() {
  if grep -q "/etc/nginx/sites/ssl/dummy/$1" "/etc/nginx/conf.d/local.conf"; then
    echo "Switching Nginx to use Let's Encrypt certificate for $1"
    sed -i "s|/etc/nginx/sites/ssl/dummy/$1|/etc/letsencrypt/live/$1|g" "/etc/nginx/conf.d/local.conf"
  fi
}
